...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($password != $confirmar) {
        ...
        $verificar = mysqli_fetch_assoc(mysqli_query($conexion, "SELECT id FROM usuarios WHERE email = '$email'"));
        if ($verificar) {
            $error = 'Ese email ya está registrado';
        } else {
            $hash = password_hash($password, PASSWORD_DEFAULT);
            mysqli_query($conexion, "INSERT INTO usuarios (nombre, email, password) VALUES ('$nombre', '$email', '$hash')");
...
<head>
  ...
  <title>Agregar Usuario — Andromeda Joyas</title>
...
<div class="main">
  ...
  <main class="content">
    <h1 class="page-title">Agregar <span>Usuario</span></h1>
